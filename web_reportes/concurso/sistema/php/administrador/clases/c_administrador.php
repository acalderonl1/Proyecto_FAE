...
class Administrador
{
            ...
            if ($altura < $minimo['width'] || $achura < $minimo['height']) {
                ...
                if ($data) {
                    ...
                    if (move_uploaded_file($_FILES['foto']['tmp_name'], $carpeta . $nombreFinal)) {
                        // $tb_administradores ='1';
                        $tb_administradores = Conexion::UpdateRegistro("INSERT INTO tb_administradores  (nombres,apellidos,celular,fecha_creado,usuario_creado)  VALUES  ('" . htmlentities($nombres) . "','" . htmlentities($apellidos) . "','" . htmlentities($celular) . "', now(),'" . htmlentities($iduser) . "')");
                        if ($tb_administradores) {
                            $pass           = sha1($password);
    ...
    {
        $tb_administradores = Conexion::UpdateRegistro("update tb_administradores set estado = '$estado',  
    ...
    function ActualizarInfoPersonal($datos)
    {
        ...
        $tb_administradores  = Conexion::UpdateRegistro("UPDATE tb_administradores set 
    ...
    function EliminarCuenta($datos)
    {
        ...
        $cod_usuario    = $_POST['cod_usuario'];
        $cod_rol        = $_POST['cod_rol'];
        $tb_administradores = Conexion::UpdateRegistro("UPDATE tb_usuarios set estado = 'D', usuario_eliminado = '$iduser', fecha_eliminado = now() 
         where cod_usuario = '$cod_usuario' and cod_rol = '$cod_rol' ");
        if ($tb_administradores) {
            $respuesta['status'] = "correcto";
    ...
    function ActualizarEstado($datos)
    {
        ...
        $cod_rol        = $_POST['cod_rol'];
        $estado         = $_POST['estado'];
        $tb_administradores = Conexion::UpdateRegistro("UPDATE tb_usuarios set estado = '$estado', usuario_editado = '$iduser', fecha_editado = now() 
         where cod_usuario = '$cod_usuario' and cod_rol = '$cod_rol' ");
        if ($tb_administradores) {
            $respuesta['status'] = "correcto";
            $respuesta['mensaje'] = "Estado actualizado";
